...
public static class TutorialContent
{
    ...
    static readonly Dictionary<string, TutorialEntry> Entries = new Dictionary<string, TutorialEntry>
    {
        ...
        [Reward] = new TutorialEntry(Reward, SectionProgress, "Награда за комнату",
            "За победу в обычном бою дают валюту забега, а сундук с предметом выпадает с вероятностью 50%. Босс гарантирует сундук не ниже редкого.\n\n" +
        ...
        [Camp] = new TutorialEntry(Camp, SectionRun, "Привал",
            ...
            "Если выбрано приготовленное блюдо, его порция тратится дополнительно и не заменяет рацион. Эффект блюда включается после лечения текущего привала и действует следующие 3 комнаты.\n\n" +
        ...
        [Boss] = new TutorialEntry(Boss, SectionCombat, "Босс этажа",
            ...
            "Победа закрывает этаж. Ветеран создаётся только после полного прохождения подземелья."),
    ...
    public const string TooltipRations = "Рацион нужен для привала даже при выбранном блюде. Блюдо тратится дополнительно; отказ ничего не тратит.";
    ...
    public const string TooltipGrade = "Ранг ветерана определяют скрытые боевые испытания итоговой сборки после полного прохождения. Число этажей само по себе ранг не задаёт.";
    ...
    public static string RoomTypeHint(RoomType type)
    {
        switch (type)
        {
            case RoomType.Combat: return "Бой. Победа даёт валюту забега; сундук с предметом выпадает с вероятностью 50%.";
