...
public class CombatantRuntime
{
    ...
    // которого обычная атака не начинается. Снижает ДПС, это намеренно.
    //
    // R05 (ФИКС): раньше это был bool, который выставлял и снимал UI по длительности флипбука
    // анимации. Механика зависела от наличия анимации: CombatSimulationEngine (аттестация) UI не
    // имеет, блокировку не получал и на том же seed выдавал лишние автоатаки, а героини без
    // анимированного спрайта не теряли ДПС там, где Дженифер теряла. Теперь длительность задаёт
    // боевая модель (CombatManager.TryActivateCooldownSkill) и тикает сама — UI только рисует.
    public float AttackLockRemaining;
    public bool IsAttackLocked => AttackLockRemaining > 0f;
    ...
    // Был ли у этого участника группы ЖИВОЙ союзник хоть раз. Без этого флага «остался один»
    // защёлкивается на первом же тике у босса, чья группа наполняется спавном по ходу боя
    // (Паучиха): на старте она в группе одна, союзник появляется только с первой кладкой.
    public bool BossGroupAllyEverSeen;
    ...
    // Boss framework (Амальгам, Паучиха, план 8): миньон — сущность, поставленная на сцену ПО ХОДУ
    // боя способностью босса, а не собранная фабрикой на старте. Отличать их обязательно по двум
    // причинам: потолок одновременно живых считается только по ним, и они умирают вместе с боссом
    ...
    public int SourceFloorNumber = 1;
